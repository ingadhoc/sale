...
class SaleOrderLine(models.Model):
    ...
    def _check_existing_gathering_products(self):
        for rec in self.filtered(lambda x: x.order_id.is_gathering and x.order_id.state == "sale"):
            existing_product_ids = (
                ...
                .search([("order_id", "=", rec.order_id.id), ("initial_qty_gathered", ">", 0)])
                .mapped("product_template_id.id")
            )
            if rec.product_template_id.id in existing_product_ids:
                raise UserError(
                    _(
                        "You can't add an already gathered product more than once. Please modify the quantity of the existing line."
                    )
                )
    ...
    def _compute_price_unit(self):
        ...
        for rec in self.filtered(
            lambda x: x.order_id.is_gathering and x.order_id.state == "sale" and x.initial_qty_gathered == 0
        ):
    ...
    def _compute_name(self):
        ...
        for line in self.filtered(
            lambda x: x.order_id.is_gathering
            and x.order_id.state == "sale"
            and x.initial_qty_gathered == 0
