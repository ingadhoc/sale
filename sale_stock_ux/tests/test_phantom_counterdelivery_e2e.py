...
class TestPhantomCounterdeliveryE2E(TransactionCase):
    ...
    def _wh(self, code, steps):
        wh = self.env["stock.warehouse"].create({"name": "Phantom %s" % code, "code": code, "delivery_steps": steps})
    ...
    def _assert_detector_matches_chain(self, order):
        ...
        expected = self._chain_reverse_alive(order).filtered(lambda m: m.to_refund and not m.origin_returned_move_id)
