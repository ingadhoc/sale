...
class SaleOrderDiscount(models.TransientModel):
    ...
    def _compute_discount_percentage(self):
        for rec in self:
            ...
            rec.discount_percentage = round(
                1.0 - discount_factor, self.env["decimal.precision"].precision_get("Discount") + 2
            )
    ...
    def action_apply_discount(self):
        ...
        if self.discount_type == "sol_discount":
            self.sale_order_id.order_line.write(
                {
                    "discount1": self.discount1 * 100,
                    "discount2": self.discount2 * 100,
                    "discount3": self.discount3 * 100,
                }
            )
