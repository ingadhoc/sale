...
class SaleOrder(models.Model):
    ...
    def _compute_sale_type_id(self):
        ...
        if user_type and isinstance(self.id, api.NewId):
            for rec in self:
                ...
                if (
                    (not user_type.company_id or user_type.company_id == rec.company_id)
                    and not rec.partner_id.with_company(rec.company_id.id).sale_type
                    and not rec.partner_id.commercial_partner_id.with_company(rec.company_id.id).sale_type
                ):
                    sales += rec
