    ...
    def test_sale_order_lot_selection(self):
        ...
        self.env["stock.move"].create(
            {
                ...
                "product_uom_id": self.product_57.uom_id.id,
        ...
        self.env["stock.move"].create(
            {
                ...
                "product_uom_id": self.product_46.uom_id.id,
                "picking_id": picking_out.id,
                "location_id": self.stock_location.id,
                "location_dest_id": self.customer_location.id,
            }
        ...
        for ops in picking_in.move_lines:
            if ops.product_id == self.product_57:
                ...
                ops.move_line_ids.write({"lot_id": lot10.id, "qty_done": ops.product_qty})
            ...
            if ops.product_id == self.product_12:
                ...
                ops.move_line_ids.write({"lot_id": lot12.id, "qty_done": ops.product_qty})
        picking_in._action_done()

        # check quantities
        lot10_qty_available = self._stock_quantity(self.product_57, lot10, self.stock_location)
        self.assertEqual(lot10_qty_available, 1)
        lot11_qty_available = self._stock_quantity(self.product_46, lot11, self.stock_location)
        self.assertEqual(lot11_qty_available, 2)
        lot12_qty_available = self._stock_quantity(self.product_12, lot12, self.stock_location)
        ...
        self.assertEqual(available_lot_ids2, lot11)

        picking_move_line_ids = picking.move_lines[0].move_line_ids
        ...
        picking.move_lines.mapped("move_line_ids").write({"qty_done": 1})
        picking.button_validate()
